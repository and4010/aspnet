	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN o.CTR_ORG_ID IS NULL OR h.CTR_HEADER_ID IS NULL THEN N'櫃號資料不存在!!' 
				WHEN h.STATUS = 2 THEN N'櫃號已取消，無法修改!!' 
				WHEN h.STATUS = 0 THEN N'櫃號已入庫，無法修改!!' 
				ELSE '' END +
		...
		BEGIN 
			...
			DELETE d FROM CTR_PICKED_T d
				JOIN @detailTable t ON t.CTR_DETAIL_ID = d.CTR_DETAIL_ID
			
			DELETE d FROM CTR_DETAIL_T d  
				JOIN @detailTable t ON t.CTR_HEADER_ID = d.CTR_HEADER_ID

			DELETE d FROM CTR_HEADER_T d
				JOIN @detailTable t ON t.CTR_HEADER_ID = d.CTR_HEADER_ID

			DELETE d FROM CTR_ORG_T d
				JOIN @detailTable t ON t.PROCESS_CODE = d.PROCESS_CODE AND t.SERVER_CODE = d.SERVER_CODE AND t.BATCH_ID = d.BATCH_ID
