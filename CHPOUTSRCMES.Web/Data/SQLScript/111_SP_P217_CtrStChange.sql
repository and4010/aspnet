...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO @detailTable (
			PROCESS_CODE, SERVER_CODE, BATCH_ID, BATCH_LINE_ID, CONTAINER_NO,
			STATUS_CODE, ERROR_MSG)
		SELECT 
			c.PROCESS_CODE, c.SERVER_CODE, c.BATCH_ID, c.BATCH_LINE_ID, c.CONTAINER_NO,
			CASE WHEN c.ATTRIBUTE1 = 'Y' 
		...
		BEGIN 
			...
			WHERE O.CONTAINER_NO IN (SELECT DISTINCT CONTAINER_NO FROM @detailTable)
			...
			DELETE d FROM CTR_HEADER_T d
				JOIN @orginTable t ON t.CTR_HEADER_ID = d.CTR_HEADER_ID

			DELETE d FROM CTR_ORG_T d
				JOIN @orginTable t ON t.PROCESS_CODE = d.PROCESS_CODE AND t.SERVER_CODE = d.SERVER_CODE AND t.BATCH_ID = d.BATCH_ID AND t.BATCH_LINE_ID = d.BATCH_LINE_ID
