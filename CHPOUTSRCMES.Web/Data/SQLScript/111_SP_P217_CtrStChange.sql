...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN 
			...
			SELECT 
				O.PROCESS_CODE, O.SERVER_CODE, O.BATCH_ID, O.BATCH_LINE_ID, 
				O.CTR_ORG_ID, D.CTR_HEADER_ID, D.CTR_DETAIL_ID 
			FROM CTR_ORG_T O 
			...
			JOIN @detailTable T ON T.CONTAINER_NO = H.CONTAINER_NO AND T.LINE_ID = D.LINE_ID
			
			-- 檢查正常，刪除主檔
			DELETE d FROM CTR_PICKED_T d
				JOIN CTR_DETAIL_T c ON d.CTR_DETAIL_ID = c.CTR_DETAIL_ID
				JOIN @deleteTable t ON t.CTR_DETAIL_ID = c.CTR_DETAIL_ID
			

			DELETE d FROM CTR_DETAIL_T d
				JOIN @deleteTable t ON t.CTR_DETAIL_ID = d.CTR_DETAIL_ID
