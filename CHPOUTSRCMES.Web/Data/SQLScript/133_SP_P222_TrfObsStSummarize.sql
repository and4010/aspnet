...
-- =============================================
CREATE PROCEDURE [dbo].[SP_P222_TrfObsStSummarize]
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			, -1
		FROM XXIF_CHP_P222_SUB_TRANSFER_ST A
		...
		SELECT 
			TRANSFER_OBSOLETE_HEADER_ID, @processCode AS PROCESS_CODE, @serverCode AS SERVER_CODE, @batchId AS BATCH_ID, NULL, @user, GETDATE()
		FROM TRF_OBSOLETE_HEADER_T H
		WHERE H.TRANSFER_OBSOLETE_HEADER_ID = @trfObsHeaderId 
		GROUP BY TRANSFER_OBSOLETE_HEADER_ID

		IF(@@ROWCOUNT <= 0 AND @@ERROR <> 0)
		BEGIN
			SET @message = @message + ' 更新 TRF_OBSOLETE_SOA_T 錯誤'
			RAISERROR(@message, 16, @success)
		END
