...
BEGIN TRANSACTION
	...
	@user VARCHAR(128)
		...
		SET @processCode = N'XXIFP219'
		SET @serverCode = N'FTY'
		SET @batchId = N'20200922131542875083'
		SET @user = N'1'
AS
BEGIN
	...
	BEGIN TRY
		...
		BEGIN -- 檢查正常，寫入主檔
			...
			SELECT
				...
				, c.BASIC_WEIGHT, c.SPECIFICATION, c.GRAIN_DIRECTION, c.ORDER_WEIGHT, c.REAM_WT
				, c.PACKING_TYPE, c.PLAN_QTY, c.WIP_PLAN_QTY, c.DTL_UOM, i.CATALOG_ELEM_VAL_020
			...
			SELECT 
				...
				, MIN(o.PE_LAST_UPDATE_DATE) AS PE_LAST_UPDATE_DATE, '0' AS [STATUS], NULL AS CUTTING_DATE_FROM, NULL AS CUTTING_DATE_TO, MIN(m.MACHINE_CODE) AS MACHINE_CODE
				, ( SELECT  OSP_REMARK + '' FROM @orgTable  d 
					WHERE d.PROCESS_CODE = o.PROCESS_CODE AND d.SERVER_CODE = o.SERVER_CODE AND d.BATCH_ID = o.BATCH_ID
					GROUP BY d.PROCESS_CODE, d.SERVER_CODE, d.BATCH_ID, d.OSP_REMARK 
					FOR XML PATH('')) AS NOTE, 0 AS MODIFICATIONS, 'SYS' AS CREATED_BY, MIN(o.CREATION_DATE) AS CREATION_DATE, NULL AS LAST_UPDATED_BY
