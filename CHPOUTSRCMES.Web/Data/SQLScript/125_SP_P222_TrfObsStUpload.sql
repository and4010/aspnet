...
-- =============================================
CREATE PROCEDURE [dbo].[SP_P222_TrfObsStUpload]
	@trfObsHeaderId BIGINT,
	@code INT OUTPUT,
...
BEGIN
	...
	DECLARE @table TABLE (
		...
		, TRANSFER_OBSOLETE_HEADER_ID BIGINT
		, TRANSACTION_DATE DATETIME
	)

	DECLARE @success INT = 0
	DECLARE @count INT = 0
	SELECT @message = 'TRANSFER_OBSOLETE_HEADER_ID:' + CONVERT(varchar, @trfObsHeaderId)
	...
	BEGIN TRY
		...
		OUTPUT inserted.PROCESS_CODE, inserted.SERVER_CODE, inserted.BATCH_ID, inserted.BATCH_LINE_ID, @trfObsHeaderId, inserted.TRANSACTION_DATE
		INTO @table (PROCESS_CODE, SERVER_CODE, BATCH_ID, BATCH_LINE_ID, TRANSFER_OBSOLETE_HEADER_ID, TRANSACTION_DATE)
		SELECT 
		...
		JOIN [TRF_OBSOLETE_HT] D ON D.TRANSFER_OBSOLETE_HEADER_ID = H.TRANSFER_OBSOLETE_HEADER_ID
		...
		JOIN [ITEMS_T] I ON I.INVENTORY_ITEM_ID = D.INVENTORY_ITEM_ID
 		WHERE H.TRANSFER_OBSOLETE_HEADER_ID = @trfObsHeaderId
		GROUP BY H.TRANSFER_OBSOLETE_HEADER_ID, D.INVENTORY_ITEM_ID, D.LOT_NUMBER
		
		IF (@@ROWCOUNT <= 0 AND @@ERROR <> 0)
		BEGIN
			SET @message = 'PROCESS_CODE:' + @processCode + ' SERVER_CODE:' + @serverCode + ' BATCH_ID:' 
			+ @batchId + ' TRANSFER_OBSOLETE_HEADER_ID:' + CONVERT(varchar, @trfObsHeaderId) + ' 上傳存貨報廢失敗'
		...
		BEGIN
			SET @message = 'PROCESS_CODE:' + @processCode + ' SERVER_CODE:' + @serverCode + ' BATCH_ID:' 
			+ @batchId + ' TRANSFER_OBSOLETE_HEADER_ID:' + CONVERT(varchar, @trfObsHeaderId) + ' 寫入 XXIF_CHP_CONTROL_ST 失敗'
			RAISERROR(@message, 16, @success)
		END

		INSERT INTO TRF_OBSOLETE_SOA_T (
			TRANSFER_OBSOLETE_HEADER_ID, PROCESS_CODE, SERVER_CODE, BATCH_ID, STATUS_CODE, CREATED_BY, CREATION_DATE
		) 
		SELECT 
			TRANSFER_OBSOLETE_HEADER_ID, PROCESS_CODE, SERVER_CODE, BATCH_ID, NULL, @user, @txnDate
		FROM @table 
		GROUP BY TRANSFER_OBSOLETE_HEADER_ID, PROCESS_CODE, SERVER_CODE, BATCH_ID
