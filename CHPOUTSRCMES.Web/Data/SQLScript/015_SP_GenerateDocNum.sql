...
BEGIN
	...
	DECLARE @docPrefix VARCHAR(50) =  '' 
	DECLARE @sequence INT = 0
	SET @docPrefix = @subinventory + @dstSubinventory + CONVERT(VARCHAR, GETDATE(), 12)
	...
	BEGIN TRY
		...
		--取得目前系統單號最大值
		SELECT @sequence = MAX(ISNULL(DOC_SEQ,0)) FROM DOC_UNIQUE_T WHERE DOC_PREFIX = @docNo GROUP BY DOC_PREFIX 
		...
		SET @docNo = @docPrefix + Replace(Str(@sequence, @digit), ' ' , '0')
