...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			SET @message = 'PROCESS_CODE:' + @processCode + ' SERVER_CODE:' + @serverCode + ' BATCH_ID:' 
			+ @batchId + ' TRANSFER_MISCELLANEOUS_HEADER_ID:' + CONVERT(varchar, @trfMiscHeaderId) + ' 寫入 XXIF_CHP_CONTROL_ST 失敗'
			RAISERROR(@message, 16, @success)
		END

		INSERT INTO TRF_MISCELLANEOUS_SOA_T (
			TRANSFER_MISCELLANEOUS_HEADER_ID, PROCESS_CODE, SERVER_CODE, BATCH_ID, STATUS_CODE, CREATED_BY, CREATION_DATE
		) 
		SELECT 
			TRANSFER_MISCELLANEOUS_HEADER_ID, PROCESS_CODE, SERVER_CODE, BATCH_ID, NULL, @user, @txnDate
		FROM @table 
		GROUP BY TRANSFER_MISCELLANEOUS_HEADER_ID, PROCESS_CODE, SERVER_CODE, BATCH_ID
