...
BEGIN
	...
	BEGIN TRY
		...
		JOIN (
			SELECT
			S.PROCESS_CODE, S.SERVER_CODE, S.BATCH_ID, S.ITEM_NO
			, SUM(S.TRANSACTION_QUANTITY) AS PQTY
			, SUM(S.SECONDARY_TRANSACTION_QUANTITY) AS SQTY
			FROM @dstTable S
			WHERE S.PROCESS_CODE = @processCode AND S.SERVER_CODE = @serverCode AND S.BATCH_ID = @batchId
			GROUP BY S.PROCESS_CODE, S.SERVER_CODE, S.BATCH_ID, S.ITEM_NO
		) A ON A.PROCESS_CODE = D.PROCESS_CODE AND A.SERVER_CODE = D.SERVER_CODE AND A.BATCH_ID = D.BATCH_ID AND A.ITEM_NO = D.ITEM_NO
