	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN c.BATCH_TYPE= 'BTH' AND c.PO_NUMBER IS NOT NULL AND c.PO_STATUS <> 'APPROVED' THEN '又裁又代 訂單狀態必須為APPROVED'
				...
				ELSE '' END +
			CASE WHEN c.BATCH_TYPE= 'BTH' AND c.PO_NUMBER IS NULL AND M1.PO_STATUS <> 'APPROVED' THEN '又裁又代 訂單狀態必須為APPROVED'
				WHEN c.BATCH_TYPE = 'BTH' AND c.PO_NUMBER IS NULL AND M1.BATCH_ID IS NULL THEN '又裁又代 未存在裁切工單資料' 
				WHEN  c.BATCH_TYPE = 'BTH' AND c.PO_STATUS = 'APPROVED' AND M1.BATCH_LINE_ID < O.BATCH_LINE_ID THEN '又裁又代 代紙單BATCH_LINE_ID 小於裁切工單'
				ELSE '' END
			AS ERROR_MSG
		  ...
		  LEFT JOIN [dbo].[XXIF_CHP_P219_OSP_BATCH_ST] M1 ON c.PROCESS_CODE = M1.PROCESS_CODE AND c.SERVER_CODE = M1.SERVER_CODE AND c.BATCH_ID = M1.BATCH_ID 
					AND M1.BATCH_NO = c.ATTRIBUTE7 AND M1.PO_NUMBER IS NOT NULL AND M1.LINE_TYPE = c.LINE_TYPE AND M1.BATCH_STATUS = c.BATCH_STATUS
