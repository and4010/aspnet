...
BEGIN
	...
	DECLARE @processCode VARCHAR(20) = 'XXIFP221', @serverCode VARCHAR(20) = 'FTY'
	...
	BEGIN TRY
		...
		PROCESS_CODE, SERVER_CODE, BATCH_ID, BATCH_LINE_ID, STATUS_CODE
			...
			, TRANSACTION_DATE, DELIVERY_DETAIL_ID, INVENTORY_ITEM_ID, TRANSACTION_QUANTITY, TRANSACTION_UOM
			, PRIMARY_QUANTITY, PRIMARY_UOM, SECONDARY_QUANTITY, SECONDARY_UOM, ROLL_NUMBER
			, ROLL_QUANTITY, REMARK, ATTRIBUTE1, ATTRIBUTE2, ATTRIBUTE3
		...
		SELECT 
			...
			, @txnDate, D.DELIVERY_DETAIL_ID, MIN(D.INVENTORY_ITEM_ID), MIN(D.REQUESTED_TRANSACTION_QUANTITY), MIN(P.TRANSACTION_UOM)
			, MIN(D.REQUESTED_PRIMARY_QUANTITY), MIN(P.PRIMARY_UOM), MIN(D.REQUESTED_SECONDARY_QUANTITY), MIN(P.SECONDARY_UOM), CASE WHEN MIN(S.ITEM_CATEGORY) = '捲筒' THEN P.LOT_NUMBER ELSE NULL END
			, CASE WHEN MIN(S.ITEM_CATEGORY) = '捲筒' THEN MIN(P.LOT_QUANTITY) ELSE NULL END, MIN(H.NOTE) 
			, NULL, NULL, NULL
		...
		BEGIN
			SET @message = 'PROCESS_CODE:' + @processCode + ' SERVER_CODE:' + @serverCode + ' BATCH_ID:' 
			+ @batchId + ' TRIP_ID:' + CONVERT(varchar, @tripId) + ' 寫入 XXIF_CHP_CONTROL_ST 失敗'
			RAISERROR(@message, 16, @success)
		END
		...
		SELECT 
			TRIP_ID, PROCESS_CODE, SERVER_CODE, BATCH_ID, @user, @txnDate
		FROM @table 
		GROUP BY TRIP_ID, PROCESS_CODE, SERVER_CODE, BATCH_ID
