...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN o.DLV_ORG_ID IS NULL 
		...
		SET @success = @success + 1
		IF EXISTS (SELECT TOP 1 * FROM DLV_HEADER_T WHERE TRIP_ID IN (SELECT DISTINCT TRIP_ID FROM @orgTable) AND DELIVERY_STATUS_CODE = 'DH5')
		BEGIN
			UPDATE @controlStage SET STATUS_CODE= 'E', ERROR_MSG = ERROR_MSG + '航程號已出貨!!' 
			IF(@@ROWCOUNT <= 0 AND @@ERROR <> 0)
			BEGIN
				SET @message = @message + ' 取消航程號錯誤'
				RAISERROR(@message, 16, @success)
		...
		BEGIN 
			...
			SELECT 
				...
				, 'SYS' AS CREATED_BY, 'SYS' CREATE_USER_NAME, MIN(o.CREATION_DATE) AS CREATION_DATE, NULL AS LAST_UPDATED_BY, NULL AS LAST_UPDATE_USER_NAME
			...
			SELECT
				...
				, p.BATCH_TYPE, t.INVENTORY_ITEM_ID, t.ITEM_NUMBER, t.ITEM_DESC_TCH, 'SYS' AS CREATED_BY 
				, 'SYS' CREATE_USER_NAME, h.CREATION_DATE, NULL AS LAST_UPDATED_BY , NULL AS LAST_UPDATE_USER_NAME, NULL AS LAST_UPDATE_DATE
			FROM DLV_ORG_T o 
