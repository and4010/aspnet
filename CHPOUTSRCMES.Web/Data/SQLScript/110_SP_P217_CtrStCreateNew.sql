...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN -- 檢查正常，寫入主檔
			...
			--檔頭
			SET @success = @success + 1
			IF NOT EXISTS (SELECT TOP 1 * FROM CTR_HEADER_T H
				JOIN CTR_ORG_T O ON O.CONTAINER_NO = H.CONTAINER_NO
				WHERE CTR_ORG_ID IN (SELECT CTR_ORG_ID FROM @table))
			BEGIN
				...
				SELECT 
					MIN(HEADER_ID), MIN(ORG_ID), MIN(ORG_NAME), MIN(BL_NO), MIN(LINE_ID), CONTAINER_NO 
					, MIN(MV_CONTAINER_DATE), MIN(ORGANIZATION_ID), MIN(ORGANIZATION_CODE), MIN(SUBINVENTORY), 1 
					,'SYS', 'SYS', MIN(CREATION_DATE), NULL, NULL
					, NULL
				FROM CTR_ORG_T o 
