...
BEGIN
	...
	BEGIN TRY
		...
		JOIN XXIF_CHP_P210_IN_MMT_INGR_ST ST ON ST.PROCESS_CODE = S1.PROCESS_CODE AND ST.SERVER_CODE = S1.SERVER_CODE AND ST.BATCH_ID = S1.BATCH_ID AND ST.ITEM_NO = I.ITEM_NUMBER
		...
		SELECT
			...
			, CASE MIN(I.CATALOG_ELEM_VAL_070) WHEN '捲筒' THEN MAX(P.PRIMARY_QUANTITY) - ISNULL(MAX(P.REMAINING_QUANTITY), 0) ELSE SUM(P.PRIMARY_QUANTITY) - SUM(ISNULL(P.REMAINING_QUANTITY, 0)) END
			, MIN(D.TRANSACTION_UOM)
			, CASE MIN(I.CATALOG_ELEM_VAL_070) WHEN '捲筒' THEN MAX(P.SECONDARY_QUANTITY) ELSE SUM(P.SECONDARY_QUANTITY) END
			, MIN(P.SECONDARY_UOM)
			, ISNULL(MIN(H.LAST_UPDATE_DATE), MIN(H.CREATION_DATE)), NULL, 'C'
			, NULL, NULL
			, dbo.ROLL_NUMBER(MIN(I.CATALOG_ELEM_VAL_070), P.LOT_NUMBER) 
			, dbo.ROLL_QUANTITY(MIN(I.CATALOG_ELEM_VAL_070), MAX(P.PRIMARY_QUANTITY) - ISNULL(MAX(P.REMAINING_QUANTITY), 0))
			, NULL
