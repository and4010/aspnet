...
BEGIN
	...
	BEGIN TRY
		...
		SELECT TOP 1 @orgOspHeaderId FROM OSP_HEADER_MOD_T WHERE OSP_HEADER_ID = @ospHeaderId
		...
		SELECT 
		ST.PROCESS_CODE, ST.SERVER_CODE, ST.BATCH_ID, BATCH_LINE_ID, NULL
		, NULL, ST.ORGCODE, 0, ST.PREVIOUS_RXID, ST.BATCH_NO
		, ST.ITEM_NO, ST.SUBINVENTORY_CODE, ST.LOCATOR, ST.TRANSACTION_QUANTITY, ST.TRANSACTION_UOM
		, ST.SECONDARY_TRANSACTION_QUANTITY, ST.SECONDARY_UOM_CODE, ST.TRANSACTION_DATE, ST.LOT_NUMBER, ST.STATUS
		...
		WHERE S1.OSP_HEADER_ID = @orgOspHeaderId
		...
		SELECT
			...
			, NULL, MIN(H.ORGANIZATION_CODE), 0, 0, MIN(BATCH_NO)
			, MIN(D.INVENTORY_ITEM_NUMBER), MIN(D.SUBINVENTORY), MIN(D.LOCATOR_CODE), SUM(P.PRIMARY_QUANTITY), MIN(P.PRIMARY_UOM)
			, SUM(P.SECONDARY_QUANTITY), MIN(P.SECONDARY_UOM), @txnDate, NULL, 'C'
			, NULL, NULL, CASE MIN(I.CATALOG_ELEM_VAL_070) WHEN  '捲筒' THEN P.LOT_NUMBER ELSE NULL END, CASE MIN(I.CATALOG_ELEM_VAL_070) WHEN  '捲筒' THEN MIN(P.LOT_QUANTITY) ELSE NULL END, NULL
		...
		MERGE INTO @orgTable o 
		USING @dstTable d ON d.BATCH_LINE_ID = o.BATCH_LINE_ID 
		WHEN MATCHED THEN 
			UPDATE SET PROCESS_CODE = d.PROCESS_CODE, SERVER_CODE = d.SERVER_CODE, BATCH_ID = d.BATCH_ID, BATCH_LINE_ID = d.BATCH_LINE_ID, STATUS_CODE = NULL
				, ERROR_MSG = d.ERROR_MSG, ORGCODE = d.ORGCODE, PREVIOUS_RXID = o.RXID, BATCH_NO = d.BATCH_NO
				, ITEM_NO = d.ITEM_NO, SUBINVENTORY_CODE = d.SUBINVENTORY_CODE, LOCATOR = d.LOCATOR, TRANSACTION_QUANTITY = d.TRANSACTION_QUANTITY, TRANSACTION_UOM = d.TRANSACTION_UOM
				, SECONDARY_TRANSACTION_QUANTITY = d.SECONDARY_TRANSACTION_QUANTITY, SECONDARY_UOM_CODE = d.SECONDARY_UOM_CODE, TRANSACTION_DATE = d.TRANSACTION_DATE, LOT_NUMBER = d.LOT_NUMBER, STATUS = d.STATUS
				, TRANSACTION_ID = d.TRANSACTION_ID, ATTRIBUTE1 = d.ATTRIBUTE1, ATTRIBUTE2 = d.ATTRIBUTE2, ATTRIBUTE3 = d.ATTRIBUTE3, ATTRIBUTE4 = d.ATTRIBUTE4
				,  
