...
-- =============================================
CREATE PROCEDURE [dbo].[SP_P222_TrfMiscStSummarize]
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			TRANSFER_MISCELLANEOUS_HEADER_ID, @processCode AS PROCESS_CODE, @serverCode AS SERVER_CODE, @batchId AS BATCH_ID, NULL, @user, GETDATE()
		FROM TRF_MISCELLANEOUS_HEADER_T H
		WHERE H.TRANSFER_MISCELLANEOUS_HEADER_ID = @trfMiscHeaderId 
		GROUP BY TRANSFER_MISCELLANEOUS_HEADER_ID

		IF(@@ROWCOUNT <= 0 AND @@ERROR <> 0)
		BEGIN
			SET @message = @message + ' 更新 TRF_MISCELLANEOUS_SOA_T 錯誤'
			RAISERROR(@message, 16, @success)
		END
