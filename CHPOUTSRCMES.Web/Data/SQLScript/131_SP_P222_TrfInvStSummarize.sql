...
-- =============================================
CREATE PROCEDURE [dbo].[SP_P222_TrfInvStSummarize]
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			TRANSFER_INVENTORY_HEADER_ID, @processCode AS PROCESS_CODE, @serverCode AS SERVER_CODE, @batchId AS BATCH_ID, NULL, @user, GETDATE()
		FROM TRF_INVENTORY_HEADER_T H
		WHERE H.TRANSFER_INVENTORY_HEADER_ID = @trfInvHeaderId 
		GROUP BY TRANSFER_INVENTORY_HEADER_ID

		IF(@@ROWCOUNT <= 0 AND @@ERROR <> 0)
		BEGIN
			SET @message = @message + ' 更新 TRF_INVENTORY_SOA_T 錯誤'
			RAISERROR(@message, 16, @success)
		END
