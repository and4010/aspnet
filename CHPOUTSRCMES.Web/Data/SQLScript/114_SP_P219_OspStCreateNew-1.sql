	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN o.OSP_ORG_ID IS NULL --未轉檔
				...
				AND ( t.CATALOG_ELEM_VAL_070 = '平版' AND c.SECONDARY_QUANTITY IS NOT NULL AND c.SECONDARY_UOM IS NOT NULL ) --平版必須有次單位
				AND ( c.PRIMARY_QUANTITY IS NOT NULL AND c.PRIMARY_UOM IS NOT NULL ) --主單位
				AND ( c.TRANSACTION_QUANTITY IS NOT NULL AND c.TRANSACTION_UOM IS NOT NULL) --交易單位
				THEN 'S' 
		...
		SET @success = 10
		--資料正常 寫入進櫃主檔、檔頭及明細
		IF NOT EXISTS (SELECT TOP 1 * FROM @controlStage WHERE STATUS_CODE = 'E' OR LEN(ERROR_MSG) > 0)
		BEGIN -- 檢查正常，寫入主檔
