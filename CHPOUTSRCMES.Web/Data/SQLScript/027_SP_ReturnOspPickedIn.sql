...
-- =============================================
CREATE PROCEDURE [dbo].[SP_ReturnOspPickedIn]
...
BEGIN TRY
	...
	UPDATE S SET 
		STATUS_CODE = CASE WHEN PRIMARY_LOCKED_QTY + (P.PRIMARY_QUANTITY - P.REMAINING_QUANTITY) > 0 THEN @processPickedStatusCode ELSE @inStockStatusCode END
	...
	FROM STOCK_T S
	JOIN OSP_PICKED_IN_HT P ON P.STOCK_ID = S.STOCK_ID
	...
	SELECT 
        ...
        ,GETDATE(),NULL,NULL
	FROM STOCK_T T
	JOIN OSP_PICKED_IN_HT P ON P.STOCK_ID = T.STOCK_ID
	WHERE OSP_HEADER_ID = @headerId
