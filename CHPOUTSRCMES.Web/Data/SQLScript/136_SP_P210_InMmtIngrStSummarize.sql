...
-- =============================================
CREATE PROCEDURE [dbo].[SP_P210_InMmtIngrStSummarize]
	@ospHeaderId BIGINT,
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			, -1
		FROM XXIF_CHP_P210_IN_MMT_INGR_ST A
		...
		FROM XXIF_CHP_P210_IN_MMT_INGR_ST 
		GROUP BY PROCESS_CODE, SERVER_CODE, BATCH_ID

		IF(@@ROWCOUNT <= 0 OR @@ERROR <> 0)
		BEGIN
			SET @message = @message + ' 更新 OSP_SOA_S1_T 錯誤'
			RAISERROR(@message, 16, @success)
		END

		INSERT INTO dbo.OSP_SOA_DTL_S1_T
           (OSP_HEADER_ID, OSP_SOA_S1_ID, INVENTORY_ITEM_ID)
		SELECT
			@ospHeaderId, MIN(S.OSP_SOA_S1_ID), MIN(I.INVENTORY_ITEM_ID)
		FROM @soa S
		JOIN XXIF_CHP_P210_IN_MMT_INGR_ST D ON D.PROCESS_CODE = S.PROCESS_CODE AND D.SERVER_CODE = S.SERVER_CODE AND D.BATCH_ID = S.BATCH_ID
		JOIN ITEMS_T I ON I.ITEM_NUMBER = D.ITEM_NO
		GROUP BY D.PROCESS_CODE, D.SERVER_CODE, D.BATCH_ID, D.ITEM_NO

		UPDATE D SET RXID = SD1.OSP_SOA_DTL_S1_ID
		--SELECT SD1.OspSoaDtlS1Id, D.PROCESS_CODE, D.SERVER_CODE, D.BATCH_ID, D.BATCH_LINE_ID
		FROM XXIF_CHP_P210_IN_MMT_INGR_ST D
		JOIN ITEMS_T I ON I.ITEM_NUMBER = D.ITEM_NO
		JOIN @soa S ON S.PROCESS_CODE = D.PROCESS_CODE AND S.SERVER_CODE = D.SERVER_CODE AND S.BATCH_ID = D.BATCH_ID
		JOIN OSP_SOA_DTL_S1_T SD1 ON SD1.INVENTORY_ITEM_ID = I.INVENTORY_ITEM_ID AND SD1.OSP_SOA_S1_ID = S.OSP_SOA_S1_ID
		
		IF(@@ROWCOUNT <= 0 OR @@ERROR <> 0)
		BEGIN
			SET @message = @message + ' 更新 OSP_SOA_DTL_S1_T 錯誤'
			RAISERROR(@message, 16, @success)
		END
