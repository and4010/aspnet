...
";
        ...
        public ResultModel DelDetailData(List<long> ids)
        {
            using (var txn = this.Context.Database.BeginTransaction())
            {
                try
                {
                    var list = trfInventoryTRepositiory.GetAll().Where(x => ids.Contains(x.TransferInventoryId)).ToList();
                    ...
                    foreach (TRF_INVENTORY_T data in list)
                    {
                        ...
                        trfInventoryTRepositiory.Delete(data, true);
                        var detailList = trfInventoryTRepositiory.GetAll().FirstOrDefault(x => x.TransferInventoryHeaderId == headerId);
                        if (detailList == null)
                        {
                            var header = trfInventoryHeaderTRepositiory.GetAll().FirstOrDefault(x => x.TransferInventoryHeaderId == headerId);
                            if (header == null) throw new Exception("找不到檔頭資料");
                            trfInventoryHeaderTRepositiory.Delete(header);
                        }
                    }
                    trfInventoryHeaderTRepositiory.SaveChanges();
            ...
            using (var txn = this.Context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (TRF_INVENTORY_T data in list)
                    {
                        ...
                        trfInventoryTRepositiory.Update(data);
                    }

                    trfInventoryTRepositiory.SaveChanges();
                    txn.Commit();
        ...
        public ResultModel SaveTransactionDetail(long transactionTypeId, string userId, string userName)
        {
            using (var txn = this.Context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var headerIdList = trfInventoryHeaderTRepositiory.GetAll().AsNoTracking().Join(
                        trfInventoryTRepositiory.GetAll().AsNoTracking(),
                ...
                .Select(x => x.Key.HeaderId).ToList();
                    ...
                    foreach (long headerId in headerIdList)
                    {
                        ...
                        trfInventoryHeaderTRepositiory.Update(header);

                        var detailList = trfInventoryTRepositiory.GetAll().Where(x => x.TransferInventoryHeaderId == header.TransferInventoryHeaderId).ToList();
        ...
        public ResultModel CreateDetailForNoStock(StockInventoryDT stockInventoryDT, string userId, string userName)
        {
            using (var txn = this.Context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var header = trfInventoryHeaderTRepositiory.GetAll().FirstOrDefault(x => x.TransactionTypeId == TransactionTypeId.Chp16In &&
