...
FROM [YSZMPCKQ_T]");
    ...
    @"SELECT y.[YSZMPCKQ_ID]
	  ...
	  ,tt.ITEM_DESC_SCH as PSTYP_CHT_NAME
	  ,tt.ITEM_DESC_ENG as PSTYP_ENG_NAME
      ,y.[OSP_SUBINVENTORY]
      ,y.[PSTYP]
      ,y.[BWETUP]
      ,y.[BWETDN]
...
left join SUBINVENTORY_T st on st.ORGANIZATION_ID = y.ORGANIZATION_ID
left join ITEMS_T tt on tt.CATALOG_ELEM_VAL_020 = y.PSTYP");
                        if (ORGANIZATION_ID != "*")
                        {
