...
namespace CHPOUTSRCMES.TASK.Models.Service
{
    public class DlvStService
    {
        ...
        {
            LogInfo($"[{tasker.Name}]-{tasker.Unit}-ImportDlvStRv-開始");
            ...
            {
                LogInfo($"[{tasker.Name}]-{tasker.Unit}-ImportDlvStRv-使用者取消");
            }
            catch (Exception ex)
            {
                LogError($"[{tasker.Name}]-{tasker.Unit}-ImportDlvStRv-錯誤-{ex.Message}-{ex.StackTrace}");
            }

            LogInfo($"[{tasker.Name}]-{tasker.Unit}-ImportDlvStRv-結束");
        }
